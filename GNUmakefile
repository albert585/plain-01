...
QEMUFLAGS := -m 4G -device qemu-xhci -serial stdio -monitor telnet::45454,server,nowait -no-reboot -d int -d cpu_reset
...
run-x86_64: ovmf/ovmf-code-$(ARCH).fd $(IMAGE_NAME).iso
	qemu-system-$(ARCH) \
		-M q35 \
		-drive if=pflash,unit=0,format=raw,file=/usr/share/edk2/x64/OVMF_CODE.4m.fd,readonly=on \
